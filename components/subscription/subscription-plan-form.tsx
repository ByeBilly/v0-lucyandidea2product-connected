...
import { type SubscriptionPlanDto, SubscriptionPlanDtoSchema } from "@/lib/types/billing/subscription-plan.dto"
...
export function SubscriptionPlanForm({ initialData, onSuccess }: SubscriptionPlanFormProps) {
  ...
  const onSubmit = async (values: SubscriptionPlanFormValues) => {
    try {
      ...
      const errorMessage =
        error instanceof AppError ? error.message : isEditMode ? t("updateFailed") : t("createFailed")

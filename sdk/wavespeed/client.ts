 ...
 * A client for interacting with the WaveSpeed API
 */
export class WaveSpeedClient {
  ...
  public async post<T = any>(
    path: string,
    data?: BaseRequest<any>,
    options: RequestOptions = {},
  ): Promise<WaveSpeedResponse<T>> {
    return this._request("POST", path, data, options)
  }

  public async postModelCall(
    data: BaseRequest<any>,
    options: RequestOptions = {},
  ): Promise<WaveSpeedResponse<ModelResult>> {
  ...
  private async _request<T = any>(
    ...
    if (requestData) {
      ...
      if (!success) {
        throw new WaveSpeedAPIError("Request validation failed", "VALIDATION_ERROR", 400, {
          errors: error?.issues || [],
        })
      }
    }

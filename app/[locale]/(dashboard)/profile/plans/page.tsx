...
import type { UserSubscriptionPlanDto } from "@/lib/types/billing/user-subscription-plan.dto"
import type { SubscriptionPlanDto } from "@/lib/types/billing/subscription-plan.dto"
...
export default function ProfilePlansPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [userSubscriptionPlanData, allSubscriptionPlansData] = await Promise.all([
        ...
        {userPlan && activeSubscriptionPlan ? (
          <div className="relative w-full max-w-md mx-auto bg-slate-800/50 backdrop-blur-xl border-slate-700/50 shadow-2xl rounded-lg p-6">
            ...
            <p className="text-slate-400 text-center mb-6">
              {t("expires")}:{" "}
              {userPlan?.currentPeriodEnd ? format(new Date(userPlan.currentPeriodEnd), "yyyy-MM-dd") : t("unknown")}
            </p>

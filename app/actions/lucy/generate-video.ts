...
export const lucyGenerateVideo = dataActionWithPermission(
  ...
  async (input: GenerateVideoInput, userContext: UserContext): Promise<GenerateVideoResult> => {
    ...
    try {
      ...
      const rateLimit = await checkRateLimit(userContext.id, "videoGen")
      ...
      // Generate video
      const videoBlob = await generateVideo(input.prompt, input.aspectRatio || "16:9")
      ...
      console.error("[v0] Lucy generateVideo error:", error)
      return {
        success: false,
        cost,

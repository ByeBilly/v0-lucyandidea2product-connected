...
type FormActionWithUserFunction<S extends z.ZodType<any, any>, T> = (
  data: z.infer<S>,
  formData: FormData,
  userContext: UserContext,
) => Promise<T>

export function formActionWithPermission<S extends z.ZodType<any, any>, T>(
  permission: string,
  schema: S,
  action: FormActionWithUserFunction<S, T>,
) {
  return async (formData: FormData | Record<string, any>) => {
    ...
    if (!result.success) {
      throw new AppError("VALIDATION_ERROR", result.error.issues[0].message)
    }

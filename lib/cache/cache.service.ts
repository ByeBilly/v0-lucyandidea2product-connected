...
export class ServerCacheService implements ICacheService {
  ...
  private async initializeServerCache() {
    try {
      ...
      const memoryTtl = Number.parseInt(process.env.CACHE_MEMORY_TTL || "60000", 10)
      const memoryLruSize = Number.parseInt(process.env.CACHE_MEMORY_LRUSIZE || "5000", 10)
      ...
      if (cacheMode === "redis") {
        ...
        if (redisUrl) {
          ...
          const redisStore = createKeyvRedis(redisUrl) as any
